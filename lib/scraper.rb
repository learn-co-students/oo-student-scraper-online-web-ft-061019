...
class Scraper
  ...
  def self.scrape_index_page(index_url)
    ...
    #below returns list of student names
    ...
    #below returns list of locations
    ...
    #below returns list of student URl
    ...
    names_array.each do |name|
      final_array << {:name => name, :location => location_array[x], :profile_url => webpage_array[x]}
    x = x + 1
    end 
    final_array
  end 
  ...
  def self.scrape_profile_page(profile_url)
    ...
    doc.xpath('//div[@class="social-icon-container"]/a').map do |link|
      if !(link['href'].include?("github")) && !(link['href'].include?("linkedin")) && !(link['href'].include?("twitter"))
